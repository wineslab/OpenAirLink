...
void configure_rf_params(EmulatorContext& ctx, const RFConfig& config)
{
    ...
    std::cout << boost::format("gNB (radio0 port0) Freq: %f MHz") 
              % (ctx.radio0->get_rx_frequency(0) / 1e6) << std::endl;
    ...
    std::cout << boost::format("UE1 (radio0 port1) Freq: %f MHz") 
              % (ctx.radio0->get_rx_frequency(1) / 1e6) << std::endl;
    ...
    std::cout << boost::format("UE2 (radio1 port0) Freq: %f MHz") 
              % (ctx.radio1->get_rx_frequency(0) / 1e6) << std::endl;
    ...
    std::cout << boost::format("UE3 (radio1 port1) Freq: %f MHz") 
              % (ctx.radio1->get_rx_frequency(1) / 1e6) << std::endl;
    ...
    std::cout << boost::format("RX Gains (gNB,UE1,UE2,UE3): %.1f, %.1f, %.1f, %.1f dB")
              % config.rx_gains[0] % config.rx_gains[1] 
              % config.rx_gains[2] % config.rx_gains[3] << std::endl;
    ...
    std::cout << boost::format("TX Gains (gNB,UE1,UE2,UE3): %.1f, %.1f, %.1f, %.1f dB")
              % config.tx_gains[0] % config.tx_gains[1] 
              % config.tx_gains[2] % config.tx_gains[3] << std::endl;
...
void init_fir_filters(EmulatorContext& ctx)
{
    ...
    std::cout << boost::format("FIR filters initialized: %d taps (5ns spacing, 200ns max delay)") 
              % NUM_FIR_TAPS << std::endl;
    std::cout << boost::format("Max FIR taps supported: %d") 
              % ctx.fir_dl[0]->get_max_num_coefficients() << std::endl;
}
...
void start_streaming(EmulatorContext& ctx, double setup_time)
{
    ...
    stream_cmd.time_spec =
        ctx.graph->get_mb_controller(ctx.mb_idx)->get_timekeeper(ctx.mb_idx)->get_time_now()
        + setup_time;
